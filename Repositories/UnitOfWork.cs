...
namespace PizzaApi.Repositories
{
	...
	public class UnitOfWork : IUnitOfWork
	{
		...
		public IPizzaRepository<TEntity> GetRepository<TEntity>() where TEntity : class
		{
			// Checks if the Dictionary Key contains the Model class
			if (_repositories.Keys.Contains(typeof(TEntity)))
			{
				// Return the repository for that Model class
				return _repositories[typeof(TEntity)] as IPizzaRepository<TEntity>;
			}

			// If the repository for that Model class doesn't exist, create it
			var repository = new PizzaRepository<TEntity>(_ctx);
